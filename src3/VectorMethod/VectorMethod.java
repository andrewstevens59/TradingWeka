...
public class VectorMethod {
	...
	private DecisionNode BuildNetwork(DataSet d, int start, int end) {
		...
		if(end - start <= 1) {
			DecisionNode v = new DecisionNode(start, null, d.DimNum());
		...
		DecisionNode v = new DecisionNode(0, child_buff, d.DimNum());
		v.AssignBestRule(d, d.DataSet().size(), 10);
		v.AssignClassError(d, d.DataSet().size());
		node_buff.add(v);
	...
	private float CreateClassifier(DataSet d, int it_num, int cycle_num) {
		...
		DecisionNode d2 = new DecisionNode(0, child_buff, d.DimNum());
	...
	public float FindClassError(DataSet d, int cycle_num, int it_num) throws NumberFormatException, IOException {
		...
		for(int i=0; i<d.DimNum(); i++) {
			DecisionNode v5 = new DecisionNode(i, null, d.DimNum());
			v5.AssignBestRule(d, d.DataSet().size(), 10);
			v5.AssignClassError(d, d.DataSet().size());
			node_buff.add(v5);
		}

		DecisionNode v5 = BuildNetwork(d, 0, d.DimNum());
		v5.AssignBestRule(d, d.DataSet().size(), 10);
		v5.AssignClassError(d, d.DataSet().size());
		node_buff.add(v5);
		...
		for(int i=0; i<20; i++) {
			...
			v5 = new DecisionNode(0, child_buff, d.DimNum());
			v5.AssignBestRule(d, d.DataSet().size(), 10);
			v5.AssignClassError(d, d.DataSet().size());
			node_buff.add(v5);
		}
	...
	// This writes the classifier to file 
	public void WriteClassifier(String file_name) throws IOException {
		FileOutputStream fos = new FileOutputStream(file_name);
		DataOutputStream dos = new DataOutputStream(fos);
        
        best_node.WriteDecisionNode(dos);     
        dos.close();
	}
	
	// This writes the classifier to file 
	public void ReadClassifier(String file_name) throws IOException {
		
		FileInputStream fos = new FileInputStream(file_name);
		DataInputStream dos = new DataInputStream(fos);
		best_node = DecisionNode.ReadDecisionNode(dos);
		dos.close();
	}
